<?php
		...
		case 'inserir':
			...
			if($tipo_venda_baixa == 0)
			{
				...
				if($qtd_acessos < 0)
				{
					...
					$valor_total = retornar_preÁo_total_acesso($conn, $id_tipo_acesso, $qtd_acessos);
			...
			else if($tipo_venda_baixa == 1)
			{
				...
				//somo o baixa acesso atual + o total que sera inserido, conseguindo saber se havera divergÍncia
	...
	$n√£oFrequencias = buscarNaoFrequencia($conn, $ids_cliente);
...
<section class="content-header">
  <h1>
    Acesso R·pido
    <small class="text-small-content-header">FrequÍncias semanais do dia <span style="color: #4a4a4a;font-weight:bold;"><?php echo "$dias_semana[$dia_inicio] ($inicioSemana) ‡† $dias_semana[$dia_final] ($atualSemana)"?></span></small>
  </h1>
</section>
	    ...
	    <div class="col-xs-12">
	        <div class="box box-danger" style="padding-top:15px;">
				...
				if(is_array($topFrequencias) && count($topFrequencias) > 0)
				{
					...
					{
						//Busca os avatares masculinos, sen√£o busca os feminino
						...
						if($topFrequencia['total_frequencia'] == 1)
							...
							$texto = $topFrequencia['total_frequencia'] . " Frequ√™ncias";
				...
				if(is_array($n√£oFrequencias) && count($n√£oFrequencias) > 0)
				{
					$i = 0;
					foreach ($n√£oFrequencias as $n√£oFrequencia)
					{
						//Busca os avatares masculinos, sen√£o busca os feminino
						$imagem = buscarAvatar($n√£oFrequencia['sexo']);
							
						echo "	<div class='col-lg-4 col-xs-12 col-sm-6'>
								<div data-toggle='modal' class='small-box bg-aqua' data-cliente='$n√£oFrequencia[id_cliente]' style='background: #d4d4d4!important;cursor: auto;'>
								<div class='inner' style='max-width: 70%;'>
								<h4>$n√£oFrequencia[nome]</h4>
								</div>
